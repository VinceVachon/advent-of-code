...
const listArray = list.map((l) => l.split(""));
let listChunks = [];
const chunkSize = 3;
for (let i = 0; i < listArray.length; i += chunkSize) {
  const chunk = listArray.slice(i, i + chunkSize);
...
const findDuplicates = (items) =>
  Array.from(
    new Set(
      items.shift().filter((v) => items.every((a) => a.indexOf(v) !== -1))
    )
  );

const allDupes = listChunks.map((chunk) => findDuplicates(chunk)).flat();
